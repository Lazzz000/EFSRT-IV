...
public class AutenticacionControlador {
    ...
    public String procesarLogin(
        ...
        if (usuarioEncontrado == null || !usuarioEncontrado.isHabilitado()) {
            modelo.addAttribute("error", "Usuario no válido o deshabilitado");
            return "login";
        }

        if (!usuarioServicio.verificarContrasena(contrasena, usuarioEncontrado.getContrasena())) {
            modelo.addAttribute("error", "Contraseña incorrecta");
            return "login";
        }

        // Guarda el usuario en sesión
        sesion.setAttribute("usuarioLogueado", usuarioEncontrado);
